...
\begin{equation*}
  \theta = \Delta t\cdot\W_{MDM} = \frac Lv \cdot \frac qm GB\cdot \frac\gamma\gamma = \frac{L\gamma GB}{B\rho} = \frac L\rho\gamma\cdot G,
\end{equation*}
where $L$ is the path length inside the B-field, and $B\rho = p/q$ is the magnetic rigidity. Consequently, the longer a particle spends in magnetic fields during revolution, the greater its spin rotation angle, and hence its spin tune, defined as the number of spin rotations per revolution. Next, we will further define the link between the spin and orbital dynamics of a particle.
