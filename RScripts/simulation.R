...
P = .8; N0 = 6730
errS = 5e-2*N0*P #absolute measurement error

phi = pi/36 #initial signal phase
w0 = 3 #signal frequency
.dcs <- function(x, w = w0) N0 * (1 + P*sin(w*x + phi)) # structural model/expectation

w.g = rnorm(1, w0, .1*w0) # if we do modulated sampling, we must guess the signal frequency; this line models the guessing

Ttot = (6*pi-phi)/w0 #total measurement time
fs = 5000 #sampling frequency
t = seq(0, Ttot, by = 1/fs);#uniform sampling time 

## modulated sampling if the measurements are taken uniformly in time
t2 = seq(0, Ttot, by = .5/fs); 
t2 <- t2[.dcs(t2, w = w.g) > N0*(1 - P*.5) & .dcs(t2, w = w.g) < N0*(1 + P*.5)];
...
llply(Es, function(e){e+s -> x; rownames(x) <- names(e); x}) %>% melt ->s
names(s) <- c("Time","Trl","Sgl", "Type")
...
s %>% ddply(.(Type, Trl), .simul, .parallel=TRUE) %>% 
  mutate(B.frq = X.frq - w0, RB.frq = B.frq/w0, Type = as.factor(Type)) -> .stats
...
print(x)
c("SE ratio" = x["uni"]/x["mod"], "wg/w0" = w.g/w0) %>% print
...
ggplot(.stats, aes(Type, SE.frq)) + geom_boxplot() + theme_bw() + labs(y=expression(sigma[hat(omega)]))
