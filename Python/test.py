...
b1 = CBunch.Bunch(Npart=1000)
at = np.arange(0,1500, .05*np.pi/b1.Synch["wFreq"])
#at = np.linspace(0,2000, 1e3)

s1 = CSignal.Signal(b1, at)

s1.addNoise(3e-2)
ggplot(s1.Signal, aes(x="Time",y="ValNs")) + geom_line(aes(x="Time", y="Val"), color="gray", size=.5) + theme_bw() + geom_point(aes(x="Time",y="Val"), data=s1.specPts, col="blue")
